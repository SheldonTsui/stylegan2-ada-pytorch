...
def txt_param_parsing(txt_fname):
...
def pkl_param_parsing(pkl_fname):
...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    if dataset_name == 'THUman':
        input_images = sorted(glob(osp.join(source_dir, 'dataset', '*/*', f'disp_uv_map_{resolution}.npy')))
    elif dataset_name == 'Multi-Garment':
        input_images = sorted(glob(osp.join(source_dir, 'Multi-Garment_dataset', '12*', f'disp_uv_map_{resolution}.npy')))
    else:
...
def convert_dataset(
    ...
    for image in tqdm(input_iter, total=num_files):
        ...
        # TODO, how to convert numpy ndarray to BytesIO?
        image_bits.write(img)
        #img.save(image_bits, format='npy', compress_level=0, optimize=False)
